...
`report.md` 适合提交或归档，`report.json` 供 CI 或其他程序稳定解析，`report.html` 适合在浏览器中查看。JSON 顶层的 `selection` 会记录 `--category` 和 `--filter`，未筛选时两项均为 `null`。轻量模式不会加载 `jinja2` 或 `matplotlib`，也不会刷新已有的 HTML、PNG 文件。

`benchmark_reports/topic06/` 是运行时输出目录，受 `.gitignore` 管理，不提交到仓库。其他开发者运行测试后，会在自己的工作区生成对应代码版本和运行环境的最新报告。

GitHub Actions 每次使用当前提交重新生成报告。Job Summary 直接渲染本次结果产生的 Mermaid 指令数图表，不依赖仓库中的静态 PNG；HTML、JSON、PNG 和单用例报告统一保存在本次运行的 `topic06-benchmark-reports` Artifact 中。CI 只读取已提交的性能基线，不会自动覆盖 `baseline.json`。
