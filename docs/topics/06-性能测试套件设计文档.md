...
本测试套件用于验证 DSL 语义以及 ScratchV 编译器在多个 DSL 程序上的编译正确性、模拟结果和性能表现。测试脚本分别运行 DSLInterpreter 参考路径和真实 TinyFive 编译路径，两条结果独立记录且禁止互相回退。测试结束后会生成精简总报告、JSON 双后端能力统计和每个用例的独立报告。
...
CI 使用 `pip install -e ".[topic06-report]"` 安装完整报告依赖，并执行 `--full-report`。Job Summary 直接展示本次运行生成的 Markdown 与 Mermaid 图表；HTML、JSON、PNG、单用例报告和失败日志通过 `if: always()` 上传到 `topic06-benchmark-reports` Artifact。`benchmark_reports/topic06/` 受 `.gitignore` 管理，不提交静态快照；仓库只保留 `benchmarks/topic06/baseline.json` 作为性能比较基线，CI 不会自动覆盖它。
